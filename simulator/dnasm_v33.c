...
const char*op_name[]={
    ...
    "SIN","COS","FMA","SYNC",
    "LABEL","JMP","JZ","JNZ","JE","JNE","JLT","JGE","JGT","JLE","CMP","CALL","RET",
    "SLEEP","REAGENT"
};
...
static const char*reagent_unit[N_REAGENTS]={
    "mg","mg","ml","pmol","ml"
};
static double reagent_total[N_REAGENTS]={0,0,0,0,0};
static int reagent_count[N_REAGENTS]={0,0,0,0,0};
...
void resolve_jumps(){
    for(int i=0;i<prog_len;i++){
        if(prog[i].op==OP_JMP||prog[i].op==OP_JZ||prog[i].op==OP_JNZ||
           prog[i].op==OP_JE||prog[i].op==OP_JNE||prog[i].op==OP_JLT||
           prog[i].op==OP_JGE||prog[i].op==OP_JGT||prog[i].op==OP_JLE||
           prog[i].op==OP_CALL){
            if(prog[i].str[0]){
...
void exec(){
    ...
    while(PC>=0&&PC<prog_len&&steps<100000000){
        ...
        switch(i->op){
            ...
            case OP_JLT:if(flag_l)PC=(int)i->val-1;break;
            case OP_JGE:if(flag_g||flag_e)PC=(int)i->val-1;break;
            case OP_JGT:if(flag_g)PC=(int)i->val-1;break;
            case OP_JLE:if(flag_l||flag_e)PC=(int)i->val-1;break;
