...
enum {
    ...
    /* 【---- GPU 并行 Primitives v3.1 ----】 */
    OP_PARA,    OP_REDUCE_SUM, OP_REDUCE_MAX,
    OP_DOT,     OP_MAD,  OP_LERP,  OP_CLAMP,
    OP_SIN,     OP_COS,  OP_FMA,   OP_SYNC,
    /* 【---- Control Flow v3.2 ----】 */
    OP_LABEL,   /* 【标签 name -- 定义 标签 (no-操作 at runtime)】 */
    OP_JMP,     /* 【跳转 标签 -- unconditional 跳转】 */
    OP_JZ,      /* 【JZ 标签 -- 跳转 如果 st[0] == 0】 */
    OP_JNZ,     /* 【JNZ 标签 -- 跳转 如果 st[0] != 0】 */
    OP_JE,      /* 【JE 标签 -- 跳转 如果 相等 (after 比较)】 */
    OP_JNE,     /* 【JNE 标签 -- 跳转 如果 不等 (after 比较)】 */
    OP_CMP,     /* 【比较 st[a] st[b] -- 比较, 设置 flags】 */
    OP_CALL,    /* 【调用 标签 -- 调用 subroutine】 */
    OP_RET,     /* 【返回 -- 返回 from subroutine】 */
    N_OPS
};
const char*op_name[]={
...
void parse_line(const char*line,Inst*I){
    ...
    /* 【标签 name -- no params needed】 */
    if(I->op==OP_LABEL&&nt>=2){
        strncpy(I->str,tok[1],63);I->str[63]=0;
    }
    /* 【跳转/JZ/JNZ/JE/JNE/调用 标签 -- store 标签 name in str】 */
...
/* 【Two-pass 加载: pass 1 collects labels, pass 2 resolves jumps】 */
    ...
    for(int i=0;i<n_lines;i++){
        ...
        if(tmp.op==OP_LABEL&&tmp.str[0]){
            if(n_labels<MAX_LABELS){
                ...
                labels[n_labels].addr=prog_len; /* 【标签's own PC】 */
                n_labels++;
            }
        }
        if(tmp.op!=OP_NOP||(p&&*p&&*p!='\n'&&*p!='#'&&*p!=';'&&*p!='\r')){
            prog_len++; /* 【ALL lines with content 计数 toward PC】 */
        }
    }
}
...
void exec(){
    ...
    while(PC>=0&&PC<prog_len&&steps<100000000){
        ...
        switch(i->op){
            ...
            /* 【Control Flow v3.2】 */
            case OP_LABEL:break; /* 【no-操作 at runtime】 */
            case OP_JMP:PC=(int)i->val-1;break; /* 【-1 because PC++ after 分支】 */
            ...
            case OP_RET:
                ...
                else PC=prog_len; /* 【返回 from 主函数 = halt】 */
                break;
            case OP_NOP:break;
            case OP_HALT:return;
        }PC++;
...
int main(int argc,char**argv){
    ...
    for(int i=0;i<n_lines&&prog_len<MAX_PROG;i++){
        ...
        if(prog[prog_len].op!=OP_NOP||(p&&*p&&*p!='\n'&&*p!='#'&&*p!=';'&&*p!='\r')){
            ...
            else{prog[prog_len].op=OP_NOP;prog_len++;} /* 【标签 -> NOP, occupies slot】 */
        }
    }
    /* 【Resolve 标签 addresses】 */
    resolve_jumps();
